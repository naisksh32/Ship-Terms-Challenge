...
2. **CHM 파일 디컴파일**
    ...
    조선해양약어집_2006.CHM.chm 파일을 HTML 파일로 디컴파일
    * Windows
...
1. HTML(디컴파일된 CHM)에서 추출
    ...
    ```
    실행 후 Data/from_html.txt 파일이 생성됨

2. 단일 한글 문자 찾기 (선택)

from_html.txt 파일 내에서 한 글자짜리 한글 항목을 찾아 파싱 오류를 식별하는 데 사용
```shell
...
* Naver
    ...
    ```
    → Data/naver_iframe_parsed.csv 생성